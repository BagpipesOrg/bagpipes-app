...
async function test_interlay() {
	...
	const tx = await assethub2interlay(1984, 200000000, "0xe64afe6914886cdcfea8da5f13e1e21aa11876cfe7fdde9299bbcdbbdc3a8b19");
...
async function test_transfers() {
    ...
    assert.strictEqual(runp.toHex(), '0xe804630203000100a10f030001010068de6e1566e333753df02b2446f24e1cc2b796cfdf954dc0f39753c578e02a40030400000000e51400000000');
    ...
    assert.strictEqual(ah.toHex(), '0x0101041f0803010100c91f030001010068de6e1566e333753df02b2446f24e1cc2b796cfdf954dc0f39753c578e02a400304000002043205011f00e5140000000000');
    ...
    assert.strictEqual(ap.toHex(), '0xfc041f080301010000030001010068de6e1566e333753df02b2446f24e1cc2b796cfdf954dc0f39753c578e02a400304000002043205011f00e5140000000000');
    ...
    assert.strictEqual(runh.toHex(), '0x680489010300000300a10f0000000000e51401010200a10f000000');
    ...
    assert.strictEqual(hp.toHex(), '0x600489010300000300000000000000e5140101020000000000');
...
async function broadcast_run_tx(tx: any) {
        ...
        console.log(tx.signAndSend(sender, ({ status }) => {
            if (status.isInBlock) {
...
async function test_balances(){
  ...
  //  console.log(aha);
    ...
    const assethubusdtbalance: number = parseFloat(assethubusdtbalanceString.replace(/,/g, ''));
    assert.strictEqual(assethubusdtbalance > 0, true, 'Free balance should be more than default 0');
    ...
    assert.ok(number_improve(polko) > 0, 'Polkadot free balance check must be greater than the default 0 value');
