...
async function handlexTransfer(formData) {
    ...
    function getParachainId(chainName) {
        const chainList = listChains(); // This returns an object with chain info
        // Iterate over the chainList to find the chain with the given name
        for (const paraId in chainList) {
            const chainInfo = chainList[paraId];
            if (chainInfo.name.toLowerCase() === chainName.toLowerCase()) {
                return chainInfo.paraid; // or parseInt(paraId)
            }
        }
...
/**/
        ...
        'polkadot:bifrost': () => {
            if(delay) {
                const numberValue = Number(delay);
                if (numberValue >= 1){
                    return polkadot_to_bifrost(submittableAmount, target.address, numberValue);
                };
            };
            console.log("handlexTransfer for Polkadot to Bifrost...");
            return polkadot_to_bifrost(submittableAmount, target.address);
        },
    
        'bifrost:polkadot': () => {
            console.log("handlexTransfer for Bifrost to Polkadot...");
            return bifrost_to_polkadot(submittableAmount, target.address);
        },
    
        // Generalized bifrost_to_parachain
        'bifrost:parachain': () => {
            const paraId = getParachainId(target.chain);
            if (!paraId) {
                throw new Error(`Unsupported target parachain: ${target.chain}`);
            }
            console.log(`Generalized bifrost_to_parachain from Bifrost to ${target.chain} (paraId: ${paraId})`);
            return bifrost_to_parachain(submittableAmount, target.address, source.assetId, paraId);
        },
...
not ready yet      
  ...
  */     
    ...
    const actionKey = `${source.chain}:${target.chain}`;
    const action = reserverTransferActions[actionKey];

    if (action) {
        console.log(`Action found for ${actionKey}`);
        return action();
    } else if (source.chain === 'bifrost') {
        // Handle generalized Bifrost to parachain transfers
        const paraId = getParachainId(target.chain);
        if (!paraId) {
            throw new Error(`Unsupported target parachain: ${target.chain}`);
        }
        console.log(`Generalized bifrost_to_parachain from Bifrost to ${target.chain} (paraId: ${paraId})`);
        return bifrost_to_parachain(submittableAmount, target.address, source.assetId, paraId);
    } else if (source.chain === 'polkadot') {
        // Handle generalized Polkadot to parachain transfers
        const paraId = getParachainId(target.chain);
        if (!paraId) {
            throw new Error(`Unsupported target parachain: ${target.chain}`);
        }
        console.log(`Generalized polkadot_to_parachain from Polkadot to ${target.chain} (paraId: ${paraId})`);
        return polkadot_to_parachain(submittableAmount, target.address, paraId, delay);
    } else {
