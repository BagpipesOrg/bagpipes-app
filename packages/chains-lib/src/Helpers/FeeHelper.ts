...
async function getApi(chain: string): Promise<ApiPromise> {
  ...
  const apiInstance = await getApiInstance(chain as ChainKey);
...
export async function getPaymentInfo(
  ...
  try {
    let api = await getApiInstance(chain as ChainKey);
    ...
    if (!api.isConnected) {
      ...
      api = await getApiInstance(chain as ChainKey); // Attempt to get a fresh instance or reconnect
      if (!api.isConnected) {
        throw new Error(
    ...
    throw new Error(
      `Error getting payment info for address ${sender}: ${(error as Error).message}`
    );
  }
}
